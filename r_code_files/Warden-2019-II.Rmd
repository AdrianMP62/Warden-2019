...
ymin <- min(Gore.spr.MDS.pts.2$MDS2)*1.3
ymax <- max(Gore.spr.MDS.pts.2$MDS2)*1.3
ggplot(Gore.spr.MDS.pts.2, aes(x=MDS1, y=MDS2)) + xlim(xmin, xmax) + ylim(ymin, ymax) + coord_fixed(ratio = 1) + geom_point(size = Gore.spr.bio.4.abund$Black.Swan/250) + geom_text(aes(label = Label), cex=3, vjust=2, hjust=1.6) + theme(legend.position = "none")
```

###Stacked abundance column plots for Gore system waterbirds
```{r}
input.data3 <- read.csv("./raw_data/Warden_Gore_by_suite_06_19.csv")
Gore.stack <- input.data3
Gore.stack <- Gore.stack[Gore.stack$System=="Gore", ] #cut down to just Gore.stack wetlands
  Gore.stack <- Gore.stack[!(Gore.stack$Date == "23/02/2010" & Gore.stack$SiteCode == "WRP004"), ] #remove duplicate data for Mullet Feb 2010
...
#### Import taxonomic framework
```{r import-taxonomy-data-and-add-Group-to-Gore-data}
Taxonomy <- read.csv("./raw_data/Taxonomy.csv")
...
Gore.stack.agg$SurvOrd <- as.numeric(str_replace_all(Gore.stack.agg$SurvOrd, c("Feb-2010" = "1", "Feb-2011" = "2", "Feb-2012" = "3", "Feb-2013" = "4", "Feb-2014" = "5", "Feb-2015" = "6", "Feb-2016" = "7", "Feb-2017" = "8", "Feb-2018" = "9", "Feb-2019" = "10", "Nov-2009" = "1", "Nov-2010" = "2", "Dec-2011" = "3", "Oct-2012" = "4", "Nov-2013"= "5", "Nov-2014" = "6", "Nov-2015" = "7", "Nov-2016" = "8", "Nov-2017" = "9", "Nov-2018" = "10")))
```
...
```{r}
Gore.stack.rich <- read.csv("./raw_data/Warden_Gore_by_suite_06_19.csv")
write.csv(Gore.stack.rich, "Gore.stack.rich.csv")
Gore.stack.rich <- Gore.stack.rich[Gore.stack.rich$System=="Gore", ] #cut down to just Gore wetlands
Gore.stack.rich <- Gore.stack.rich[-grep("Unidentified", Gore.stack.rich$CommonName), ] #remove rows where birds not identified
  Gore.stack.rich$Date <- as.Date(Gore.stack.rich$Date, format="%d/%m/%Y")
  Gore.stack.rich$Survey <- Gore.stack.rich$Date #create Survey field from Date
  Gore.stack.rich$Survey <- format(as.Date(Gore.stack.rich$Survey), "%b-%Y") #change format to mmm-YYYY
  Gore.stack.rich$Survey <- gsub("Oct-2018", "Nov-2018", Gore.stack.rich$Survey) #lump survey dates that straddle Oct/Nov 2018 and call them Nov-2018
  yq <- as.yearqtr(as.yearmon(Gore.stack.rich$Date, "%Y/%m/%d")) #create a temporary season field
Gore.stack.rich$Season <- factor(format(yq, "%q"), levels = 1:4, labels = c("summer", "autumn", "winter", "spring"))
```
...
Gore.stack.rich$SurvOrd <- as.numeric(str_replace_all(Gore.stack.rich$SurvOrd, c("Feb-2010" = "1", "Feb-2011" = "2", "Feb-2012" = "3", "Feb-2013" = "4", "Feb-2014" = "5", "Feb-2015" = "6", "Feb-2016" = "7", "Feb-2017" = "8", "Feb-2018" = "9", "Feb-2019" = "10", "Nov-2009" = "1", "Nov-2010" = "2", "Dec-2011" = "3", "Oct-2012" = "4", "Nov-2013"= "5", "Nov-2014" = "6", "Nov-2015" = "7", "Nov-2016" = "8", "Nov-2017" = "9", "Nov-2018" = "10")))
```
...
WGdata <-WGdata[WGdata$SurveyType == "ground", ]
ground.data <- as.data.frame(acast(WGdata, CommonName ~ SSD, value.var="Count")) #sums counts for each species and survey
names <- row.names(ground.data)
ground.data <- as.data.frame(t(ground.data))
ground.data$SSD <- row.names(ground.data)
ground.data$Site <- substr(ground.data$SSD, start=1, stop=6)
```
