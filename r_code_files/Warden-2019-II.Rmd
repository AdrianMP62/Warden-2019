...
Packages <- c("vegan", "ggrepel", "ggplot2", "scales","lubridate", "stringr", "gsubfn", "reshape2", "knitr", "simba", "zoo", "car")
...
WG.MDS.bySyst <- metaMDS(WG.bySystem.m, distance = "bray", transform="false")
```
...
av.depths$Pipeline <- NULL #remove factor for pipeline open/close
av.depths$Pipeline_days <- NULL
av.depths <- av.depths[, -av.depths$Pipeline_days]
av.depths.spr <- av.depths[av.depths$Season == "Spring", ] #restrict to spring
av.depths.spr <- av.depths.spr[order(av.depths.spr$Survey), ]
av.depths.sum <- av.depths[av.depths$Season == "Summer", ] #restrict to summer
av.depths.sum <- av.depths.sum[order(av.depths.sum$Survey), ]
BEdat.spr <- as.data.frame(av.depths.spr[, 3:ncol(av.depths.spr)], row.names = as.character(av.depths.spr$Survey))
BEdat.sum <- as.data.frame(av.depths.sum[, 3:ncol(av.depths.sum)], row.names = as.character(av.depths.sum$Survey))
```

```{r check-correlation-between-env-variables}
cor(BEdat.spr, method="spearman")
cor(BEdat.sum, method="spearman")
```

```{r additional-test-datasets}
P <- BEdat.spr[, c(4,5)] #create a pipeline only dataset
C <- BEdat.spr[, c(7:15)] #create rainfall only dataset
C <- C+min(C)
C <- vegdist(C, method="euclidean") #create rainfall only similarity matrix
C
```
...
ggplot(Warden.sum.MDS.pts, aes(x=MDS1, y=MDS2)) + xlim(-0.35, 0.3) + ylim(-0.2, 0.2) + coord_fixed(ratio = 1) + geom_point(aes(size = BEdat.sum.st$Depth, colour=BEdat.sum.st$Depth)) + scale_size(range = c(3, 12)) + geom_text(aes(label = Survey), cex=3, vjust=2, hjust=1.25) + theme(legend.position = "none")
```
...
ggplot(Warden.spr.MDS.pts, aes(x=MDS1, y=MDS2)) + xlim(-0.3, 0.3) + ylim(-0.2, 0.25) + coord_fixed(ratio = 1) + geom_point(aes(size = BEdat.spr.st$Pipeline_days, colour = BEdat.spr.st$Pipeline_days)) + scale_size(range = c(3, 12)) + geom_text(aes(label = Survey), cex=3, vjust=2, hjust=1.25) + theme(legend.position = "none") + scale_x_reverse(limits=c(0.3, -0.3))
```

Bio-env analysis to determine influence of depth on waterbrd communities on the Warden wetlands
```{r bio-env-Warden}
#uses the multiple env variable dataset BEdat.spr and BEdat.sum rather than just the depth only datastet.
#SPRING
Warden.spr.be <- bioenv(Warden.spr.m, BEdat.spr, method = "spearman", index = "bray", metric="euclidean")
Warden.spr.be #results of bio-env for spring
Warden.spr.bed <- bioenvdist(Warden.spr.be, which = "best") #extract euclidean distance matrix for best set of env vars
Warden.spr.md <- vegdist(Warden.spr.m, method = "bray") #calculate Warden spring waterbird bray-curtis dissimilarity index
mantel(Warden.spr.md, Warden.spr.bed, method="spearman", permutations=999) #mantel test to determine permutational significance
?mantel    
#SUMMER
Warden.sum.be <- bioenv(Warden.sum.m, BEdat.sum, method = "spearman", index = "bray", metric="euclidean")
Warden.sum.be
Warden.sum.bed <- bioenvdist(Warden.sum.be, which = "best") #extract euclidean distance matrix for best set of env vars
Warden.sum.md <- vegdist(Warden.sum.m, method = "bray") #calculate Warden summer waterbird bray-curtis dissimilarity index
mantel(Warden.sum.md, Warden.sum.bed, method="spearman", permutations=999) #mantel test to determine permutational significance
```

```{r using-bv.step-function}
Warden.spr.bvs <- bv.step_ext(Warden.spr.m, BEdat.spr, fix.dist.method = "bray", var.dist.method = "euclidean", scale.fix=FALSE, scale.var=TRUE, max.rho=0.95, min.delta.rho=0.001)
Warden.spr.bvs
```
...
```{r environmental-ordination-best-vars-from-bioenv}
Warden.env.spr.MDS <- metaMDS(Warden.spr.bed)
Warden.env.spr.MDS.pts <- as.data.frame(Warden.env.spr.MDS$points) #extract coordinates from ordination
Warden.env.spr.MDS.pts$Survey <- row.names(Warden.env.spr.MDS.pts)
ggplot(Warden.env.spr.MDS.pts, aes(x=MDS2, y=MDS1)) + xlim(-5, 5) + ylim(-5, 5) + coord_fixed(ratio = 1) + geom_point(size=10) + geom_text(aes(label = Survey), cex=3, vjust=2, hjust=1.25) + theme(legend.position = "none")
```
...
Warden.spr.dbrda <- capscale(Warden.spr.m ~ depth + Condition(pipeline), BEdat.spr, dist="bray")
Warden.spr.dbrda
plot(Warden.spr.dbrda, display = c("wa", "bp", "sp"))
Warden.sum.dbrda <- capscale(Warden.sum.m ~ CDM3MO_ESPER + CDM6MO_MYRUP, BEdat.sum, dist="bray")
Warden.sum.dbrda
plot(Warden.sum.dbrda, display = c("wa", "bp", "sp"))
```

```{r stepwise-rda-model-building-spring}
M.spr.1 <- capscale(Warden.spr.m ~ ., BEdat.spr, dist="bray") #create model with all parameters
M.spr.0 <- capscale(Warden.spr.m ~ 1, BEdat.spr, dist="bray") #create model with no parameters
m <- ordistep(M.spr.0, scope=formula(M.spr.1), permutations=500) #stepwise model build
```

```{r stepwise-rda-model-building-summer}
M.sum.1 <- capscale(Warden.sum.m ~ ., BEdat.sum, dist="bray") #create model with all parameters
M.sum.0 <- capscale(Warden.sum.m ~ 1, BEdat.sum, dist="bray") #create model with no parameters
m <- ordistep(M.sum.0, scope=formula(M.sum.1), permutations=500) #stepwise model build
```



```{r norm-norm-text}
qqnorm(BEdat.sum$depth)
```
