...
Warden.sprsp.m <- Warden.spr.msq[, spring.species] #extract just the 8 species for spring
fit <- envfit(Warden.spr.MDS, Warden.sprsp.m, permutations = 999, choices=c(1,2)) #fit these species as vectors in unconstrained ordination
Warden.arrows.spr <-as.data.frame(fit$vectors$arrows*sqrt(fit$vectors$r)) #extract vector coordinates
Warden.arrows.spr$Species <- row.names(Warden.arrows.spr) #add species names to vectors

fit.env <- envfit(Warden.spr.MDS, input env data

plot(Warden.spr.MDS, display="sites", type = "t", ylim= c(-1, 5), xlim=rev(c(-0.2, 0.2)))
plot(fit)
...
points(mod3, display = "sites", pch=16, cex=Warden.sprsp.m$Straw.necked.Ibis^2/50, col="grey") #scale sites by abundances of the 8 species
...
Fit vectors from the env variables identified in the db-RDA and bio-env analysis on a plot of the constrained ordination with symbols scaled to each of the 8 species identified in the bio-bio bv.step analysis
```{r}
