...
#spring
mod1 <- capscale(Warden.spr.m ~ Pipeline_days, Surv.Pipe.spr, dist="bray") #test individual variables
mod1
anova(mod1) #signif of individual variable
mod2 <- capscale(Warden.spr.m ~ . + Condition(Surv.Pipe.spr$Pipeline_days), BEdat.spr, dist="bray") #full model
mod0 <- capscale(Warden.spr.m ~ 1, BEdat.spr, dist="bray") #Null model
ind <- ordistep(mod0, scope = formula(mod2), perm.max = 999) #stepwise model build from all
anova(ind) #signif of stepwise model
mod3 <- capscale(Warden.spr.m ~ CDM12MO_EAERO + CDM12MO_ESPER + Depth, BEdat.spr, dist ="bray") #model selected by ordistep
