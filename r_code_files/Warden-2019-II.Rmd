...
BEdat.spr <- as.data.frame(av.depths.spr[, 3:12], row.names = as.character(av.depths.spr$Survey))
BEdat.sum <- as.data.frame(av.depths.sum[, 3:12], row.names = as.character(av.depths.sum$Survey))
```
...
ggplot(Warden.sum.MDS.pts, aes(x=MDS1, y=MDS2)) + xlim(-0.35, 0.3) + ylim(-0.2, 0.2) + coord_fixed(ratio = 1) + geom_point(aes(size = BEdat.sum.st$CDM6MO_MYRUP, colour=BEdat.sum.st$CDM6MO_MYRUP)) + scale_size(range = c(3, 12)) + geom_text(aes(label = Survey), cex=3, vjust=2, hjust=1.25) + theme(legend.position = "none")
```
