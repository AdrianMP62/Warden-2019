...
Aggregate count data by just common name and survey for creating matrices
...
Extract ordination point daat for ggplots
...
ggplot(Warden.spr.MDS.pts, aes(x=MDS1, y=MDS2)) + xlim(-0.3, 0.3) + ylim(-0.2, 0.25) + coord_fixed(ratio = 1) + geom_point(aes(size = Pipeline.spr$Pipeline_days, colour = Pipeline.spr$Pipeline_days)) + scale_size(range = c(3, 12)) + geom_text(aes(label = Survey), cex=3, vjust=2, hjust=1.25) + theme(legend.position = "none") + scale_x_reverse(limits=c(0.3, -0.3))
```
...
#SPRING
#Pipeline or Survey days only
Warden.spr.be.P <- bioenv(Warden.spr.m, Pipeline.spr, method = "spearman", index = "bray", metric="euclidean")
Warden.spr.be.P
Warden.spr.md <- vegdist(Warden.spr.m, method = "bray") #calculate Warden spring waterbird bray-curtis dissimilarity index
mantel(Warden.spr.md, Pipeline.spr.m, method="spearman", permutations=999) #mantel test to determine permutational significance

#other vars
Warden.spr.be <- bioenv(Warden.spr.m, BEdat.spr, method = "spearman", index = "bray", metric="euclidean", partial = Pipeline.spr.m)
...
#SUMMER
#Pipeline or Survey days only
Warden.sum.be.P <- bioenv(Warden.sum.m, Pipeline.sum, method = "spearman", index = "bray", metric="euclidean")
Warden.sum.be.P
Warden.sum.md <- vegdist(Warden.sum.m, method = "bray") #calculate Warden spring waterbird bray-curtis dissimilarity index
mantel(Warden.sum.md, Pipeline.sum.m, method="spearman", permutations=999) #mantel test to determine permutational significance

#other vars
Warden.sum.be <- bioenv(Warden.sum.m, BEdat.sum, method = "spearman", index = "bray", metric="euclidean", partial = Pipeline.sum.m)
Warden.sum.be
