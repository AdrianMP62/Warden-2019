...
BEdat.spr <- as.data.frame(av.depths.spr[, 3:9], row.names = as.character(av.depths.spr$Survey))
BEdat.sum <- as.data.frame(av.depths.sum[, 3:9], row.names = as.character(av.depths.sum$Survey))
```
...
ggplot(Warden.sum.MDS.pts, aes(x=MDS1, y=MDS2)) + xlim(-0.35, 0.3) + ylim(-0.2, 0.2) + coord_fixed(ratio = 1) + geom_point(aes(size = D.sum, colour=D.sum)) + scale_size(range = c(3, 12)) + geom_text(aes(label = Survey), cex=3, vjust=2, hjust=1.25) + theme(legend.position = "none")
```
...
Warden.spr.be <- bioenv(Warden.spr.m, BEdat.spr.st, method = "spearman", index = "bray", metric="euclidean")
Warden.spr.be
D.sum.df <- as.data.frame(D.sum, row.names = row.names(Warden.sum.m)) #extract depths
Warden.sum.be <- bioenv(Warden.sum.m, BEdat.sum.st, method = "spearman", index = "bray", metric="euclidean")
Warden.sum.be
```

```{r dbRDA-of-Warden-data}
Warden.spr.dbrda <- capscale(Warden.spr.m ~ depth + CDM3MO_ESPER + CDM6MO_ESPER + CDM12MO_ESPER, BEdat.spr.st, dist="bray")
Warden.spr.dbrda
plot(Warden.spr.dbrda, display = c("wa", "bp"))
Warden.sum.dbrda <- capscale(Warden.sum.m ~ depth + CDM3MO_EAERO + CDM6MO_EAERO + CDM12MO_EAERO, BEdat.sum, dist="bray")
Warden.sum.dbrda
plot(Warden.sum.dbrda, display = c("wa", "bp"))
```

```{r norm-norm-text}
qqnorm(BEdat.sum.st$CDM6MO_9542)
```
