...
```{r}
#summer
Gore.sum.m.dist <- vegdist(Gore.sum.m, method = "bray")
Gore.sum.m.sqrt <- sqrt(Gore.sum.m)
Gore.sum.m.sqrt.dist <- vegdist(Gore.sum.m.sqrt, method = "bray", try=1000)
#using own distance matrix
Gore.sum.MDS.1 <- metaMDS(Gore.sum.m.sqrt.dist, k=2, autotransform = FALSE)
plot(Gore.sum.MDS.1)
text(Gore.sum.MDS.1, display = c("sites"))
stressplot(Gore.sum.MDS.1)
#supplying data matrix
Gore.sum.MDS.2 <- metaMDS(Gore.sum.m.sqrt, distance = "bray", k=2, autotransform = FALSE)
plot(Gore.sum.MDS.2)
text(Gore.sum.MDS.2, display = c("sites"))
...
#To avoid collapse of some surveys to the same ordination point (degenerate ordination)
```{r}
Gore.sum.monoMDS <- monoMDS(Gore.sum.m.sqrt.dist, k=2, model = "global", try=1000)
plot(Gore.sum.monoMDS)
stressplot(Gore.sum.monoMDS)
Gore.sum.monoMDS.pts <- Gore.sum.monoMDS$points
