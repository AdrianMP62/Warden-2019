...
```{r load-packages}
Packages <- c("vegan", "ggrepel", "grid","ggplot2", "plyr", "scales","lubridate", "stringr", "gsubfn", "reshape2", "knitr", "simba", "zoo", "car")
...
```{r new plot for Birdlife Australia talk}
ggplot(WG.3dMDS.bySyst.pts, aes(x=MDS1, y=MDS2, shape=System, colour=System)) + xlim(-0.5, 0.5) + ylim(-0.4, 0.4) + coord_fixed(ratio = 1) + geom_point(size=7) + scale_color_manual(values=c("blue","red")) + scale_shape_manual(values=c(15,17)) + theme(plot.margin=unit(c(0,0,0,0),"mm")) + theme(axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank()) + theme(axis.title.y=element_blank(), axis.text.y=element_blank(), axis.ticks.y=element_blank()) + theme(legend.title = element_blank()) + theme(legend.position = "none")
```
...
md.Group <- rep("Grebes", times = 6)
md.Survey <- c("Nov-2015","Nov-2016","Nov-2017","Feb-2016","Feb-2017","Feb-2018")
md.Season <- c(rep("spring", times = 3), rep("summer", times = 3))
md.Count <- rep(0, times = 6)
...
ggplot(Warden.stack.spr, aes(x=reorder(Survey, SurvOrd), y=Count, fill=Group)) + geom_bar(stat="identity", colour = "black") + scale_y_continuous(breaks = seq(0,30000, by=5000), limits=c(0,30000)) + labs(x="Survey", y="Abundance") + theme (axis.text.x=element_text(angle=90)) + theme(axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0))) + theme(axis.title.x = element_text(margin = margin(t = 20, r = 0, b = 0, l = 0))) + scale_fill_manual(values = c("#CD9B9B","#912CEE","#104E8B","#698B22","#00EEEE","#A2B5CD","#FFA500","#FFE1FF","#B4CDCD","#C1FFc1","#A52A2A"))
...
###Stacked column plots for species richness
```{r}
Warden.stack.rich <- read.csv("./raw_data/Warden_Gore_by_suite_06_19.csv")
write.csv(Warden.stack.rich, "Warden.stack.rich.csv")
Warden.stack.rich <- Warden.stack.rich[Warden.stack.rich$System=="Warden", ] #cut down to just Warden.stack.rich wetlands
Warden.stack.rich <- Warden.stack.rich[-grep("Grassbird", Warden.stack.rich$CommonName), ] #remove records of little grassbird
  Warden.stack.rich <- Warden.stack.rich[!(Warden.stack.rich$Date == "23/02/2010" & Warden.stack.rich$SiteCode == "WRP004"), ] #remove duplicate data for Mullet Feb 2010
  Warden.stack.rich$Date <- as.Date(Warden.stack.rich$Date, format="%d/%m/%Y")
  Warden.stack.rich$Survey <- Warden.stack.rich$Date #create Survey field from Date
  Warden.stack.rich$Survey <- format(as.Date(Warden.stack.rich$Survey), "%b-%Y") #change format to mmm-YYYY
  Warden.stack.rich$Survey <- gsub("Oct-2018", "Nov-2018", Warden.stack.rich$Survey) #lump survey dates that straddle Oct/Nov 2018 and call them Nov-2018
  yq <- as.yearqtr(as.yearmon(Warden.stack.rich$Date, "%Y/%m/%d")) #create a temporary season field
Warden.stack.rich$Season <- factor(format(yq, "%q"), levels = 1:4, labels = c("summer", "autumn", "winter", "spring"))
```
...
```{r}
duplicate.spp <- count(Warden.stack.rich, c("CommonName","Survey","SiteCode","Subsite")) #count no. species
write.csv(duplicate.spp, "./analysis_data/duplicatespp.csv")
```
...
```{r}
Warden.stack.rich.spr <- Warden.stack.rich[Warden.stack.rich$Season=="spring", ] #cut down to just spring data
Warden.stack.rich.spr <- Warden.stack.rich.spr[order(Warden.stack.rich.spr$SurvOrd), ]
ggplot(Warden.stack.rich.spr, aes(x=reorder(Survey, SurvOrd), y=freq, fill=Group)) + geom_bar(stat="identity", colour = "black") + scale_y_continuous(breaks = seq(0,50, by=10), limits=c(0,50)) + labs(x="Survey", y="Number of species") + theme (axis.text.x=element_text(angle=90)) + theme(axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0))) + theme(axis.title.x = element_text(margin = margin(t = 20, r = 0, b = 0, l = 0))) + scale_fill_manual(values = c("#CD9B9B","#912CEE","#104E8B","#698B22","#00EEEE","#A2B5CD","#FFA500","#FFE1FF","#B4CDCD","#C1FFc1","#A52A2A"))
...
ggplot(Warden.stack.rich.sum, aes(x=reorder(Survey, SurvOrd), y=freq, fill=Group)) + geom_bar(stat="identity", colour = "black") + scale_y_continuous(breaks = seq(0,50, by=10), limits=c(0,50)) + labs(x="Survey", y="Number of species") + theme (axis.text.x=element_text(angle=90)) + theme(axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0))) + theme(axis.title.x = element_text(margin = margin(t = 20, r = 0, b = 0, l = 0))) + scale_fill_manual(values = c("#CD9B9B","#912CEE","#104E8B","#698B22","#00EEEE","#A2B5CD","#FFA500","#FFE1FF","#B4CDCD","#C1FFc1","#A52A2A"))
```
...
####2D MDS unconstrained ordination of Gore surveys by each season
```{r}
#summer
...
Gore.sum.MDS.pts$Survey <- row.names(Gore.sum.MDS.pts) #add survey variable from row names
Gore.sum.MDS.pts$Survey <- as.yearmon(Gore.sum.MDS.pts$Survey, "%b-%Y") #convert survey to date format using package zoo
Gore.sum.MDS.pts <- Gore.sum.MDS.pts[order(as.Date(Gore.sum.MDS.pts$Survey, format="%b-%Y")),] #order by Survey date
...
#### Bio-bio analysis of spring waterbird species
```{r}
...
Gore.spr.bio.4.abund.2 <- Gore.spr.bio.4.abund[order(row.names(Gore.spr.bio.4.abund)), ] #create new 4 species file reordered by row names for mantel test
Gore.spr.bio.4.dist <- vegdist(Gore.spr.bio.4.abund.2, method = "bray")
Gore.spr.m.dist <- vegdist(Gore.spr.m, method = "bray")
...
#### merge site and restrcited species scores from bv.step
```{r}
Gore.spr.MDS.pts <- rbind(Gore.spr.MDS.pts, Gore.spr.bio.4) 
```
...
Gore.spr.bio.4.abund <- Gore.spr.bio.4.abund[order(as.Date(Gore.spr.bio.4.abund$Label2, format="%b-%Y")),] #order by Survey date
...
```{r}
#summer
ggplot(Gore.sum.MDS.pts, aes(x=MDS1, y=MDS2)) + xlim(-0.75, 0.5) + ylim(-0.35, 0.35) + coord_fixed(ratio = 1) + geom_point(cex=8) + geom_path() + geom_text(aes(label = Survey), cex=3, vjust=2, hjust=1.25) + theme(legend.position = "none")

#spring - with species scores
#ggplot(Gore.spr.MDS.pts, aes(x=MDS1, y=MDS2, group=Type, colour=Type, size=Type)) + xlim(-0.7, 0.5) + ylim(-0.4, 0.25) + coord_fixed(ratio = 1) + geom_point() + scale_color_manual(values=c("blue","red")) + scale_size_manual(values=c(2,6)) + geom_text(aes(label = Label), cex=3, vjust=2, hjust=1.25) + theme(legend.position = "none")

#spring with scaling of survey points to species abundance (specify symbol size with Gore.spr.bio.4.abund$[species])
ggplot(Gore.spr.MDS.pts, aes(x=MDS1, y=MDS2)) + xlim(-0.5, 0.5) + ylim(-0.3, 0.3) + coord_fixed(ratio = 1) + geom_point(cex=Gore.spr.bio.4.abund$Banded.Stilt/200) + geom_text(aes(label = Label), cex=3, vjust=2, hjust=1.6) + theme(legend.position = "none")
```
...
```{r}
Species <- read.csv("./raw_data/Warden_Gore_by_suite_06_19.csv")
Species <- Species[-grep("Unidentified", Species$CommonName), ] #remove rows where birds not identified
Species <- Species[Species$System=="Warden", ] #cut down to just Warden wetlands
Species <- Species[-grep("Grassbird", Species$CommonName), ] #remove records of little grassbird
  Species <- Species[!(Species$Date == "23/02/2010" & Species$SiteCode == "WRP004"), ] #remove duplicate data for Mullet Feb 2010
  Species$Date <- as.Date(Species$Date, format="%d/%m/%Y")
  Species$Survey <- Species$Date #create Survey field from Date
  Species$Survey <- format(as.Date(Species$Survey), "%b-%Y") #change format to mmm-YYYY
  Species$Survey <- gsub("Oct-2018", "Nov-2018", Species$Survey) #lump survey dates that straddle Oct/Nov 2018 and call them Nov-2018
  yq <- as.yearqtr(as.yearmon(Species$Date, "%Y/%m/%d")) #create a temporary season field
Species$Season <- factor(format(yq, "%q"), levels = 1:4, labels = c("summer", "autumn", "winter", "spring")) #create Season (spring vs summer) variable
