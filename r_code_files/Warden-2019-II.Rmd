...
ggplot(Warden.spr.MDS.pts, aes(x=MDS1, y=MDS2)) + xlim(-0.7, 0.7) + ylim(-0.7, 0.7) + coord_fixed(ratio = 1) + geom_point(aes(size = BEdat.spr$Depth, colour = BEdat.spr$Depth)) + scale_size(range = c(3, 12)) + geom_text(aes(label = Survey), cex=3, vjust=2, hjust=1.25) + theme(legend.position = "none") + scale_x_reverse(limits=c(0.7, -0.7))
```
...
```{r}
Warden.spr.msq <- sqrt(Warden.spr.m)
...
Warden.sprsp.m <- Warden.spr.msq[, spring.species] #spring only data
fit <- envfit(Warden.spr.MDS, Warden.sprsp.m, permutations = 999, choices=c(1,2))
