...
Warden.spr.dbrda <- capscale(Warden.spr.m ~ CDM12MO_EAERO + CDM12MO_MYRUP, BEdat.spr, dist="bray")
Warden.spr.dbrda
plot(Warden.spr.dbrda, display = c("wa", "bp"))
Warden.sum.dbrda <- capscale(Warden.sum.m ~ CDM12MO_EAERO, BEdat.sum, dist="bray")
Warden.sum.dbrda
plot(Warden.sum.dbrda, display = c("wa", "bp"))
```

```{r}
m <- ordistep(Warden.spr.dbrda, perm.max=200)
```
...
```{r}
M1 <- capscale(Warden.spr.m ~ ., BEdat.spr, dist="bray")
M0 <- capscale(Warden.spr.m ~ 1, BEdat.spr, dist="bray")
m <- ordistep(M0, scope=formula(M1), perm.max=200)
```
