...
Packages <- c("vegan", "ggplot2", "stringr", "reshape2", "knitr", "simba", "zoo", "car")
...
```{r import and restructure Warden and Gore waterbird data}
 WG.bySeason <- input.data <- read.csv("./raw_data/Warden_Gore_by_suite_06_19.csv") #csv file is a flat file of all species records by survey date, site code, system and survey method
...
```{r undertake_2D_and_3D_ordination}
...
```{r create_3D_dataset_for_plotting}
...
```{r, include=TRUE}
  ggplot(WG.3dMDS.bySeas.pts, aes(x=MDS1, y=MDS2)) + xlim(-0.4, 0.35) + ylim(-0.3, 0.3) + geom_point(aes(colour=Season), size=5) + coord_fixed(ratio = 1) + geom_text(aes(label=Survey), size=3, vjust=1, hjust=1.3)
```
...
```{r, include=TRUE}
ggplot(WG.MDS.bySyst.pts, aes(x=MDS1, y=MDS2)) + xlim(-0.6, 0.5) + ylim(-0.3, 0.4) + geom_point(aes(colour=SystSeas), size=5) + geom_text(aes(label=Survey), size=3, vjust=1, hjust=1.3)
```
...
```{r}
WG.MDS3D.bySyst <- metaMDS(WG.bySystem.m, distance = "bray", k=3)
```

```{r extract-coordinates-from-3D-nMDS-and-add-season-to-resulting-data.frame-for Warden-versus-Gore-analysis}
system <- unique(input.data2$System) #extract a unique list of systems (Gore, Warden)
WG.MDS3D.bySyst.pts <- as.data.frame(WG.MDS.bySyst.3D$points) #extract coordinates from metaMDS
WG.MDS3D.bySyst.pts$Sys_Sur <- row.names(WG.MDS3D.bySyst.pts) #use row.names to create new System+Survey period variable
WG.MDS3D.bySyst.pts$Survey <- str_sub(WG.MDS3D.bySyst.pts$Sys_Sur, -8, -1) #extract Survey period from Sys-Sur variable by extracting last 8 characters
WG.MDS3D.bySyst.pts$System <- substr(WG.MDS3D.bySyst.pts$Sys_Sur, 1, 4) #extract System from Sur-sys variable by extracting first 4 variables
WG.MDS3D.bySyst.pts$System <- gsub("Ward", "Warden", WG.MDS3D.bySyst.pts$System) #convert 'Ward' to 'Warden'
#WG.MDS3D.bySyst.pts <- WG.MDS3D.bySyst.pts[with(WG.MDS3D.bySyst.pts, order(Survey)), ] #order by survey
WG.MDS3D.bySyst.pts$Season <-  substr(WG.MDS3D.bySyst.pts$Survey, 1, 3) #extract months
WG.MDS3D.bySyst.pts$Season <- recode(WG.MDS3D.bySyst.pts$Season, "'Feb'='summer';c('Dec','Oct','Nov')='spring'") #convert months to season
WG.MDS3D.bySyst.pts$SystSeas <- paste(WG.MDS3D.bySyst.pts$System, WG.MDS3D.bySyst.pts$Season, sep=" in ")
str(WG.MDS3D.bySyst.pts) #join System and season
```

ordination plot (of 3D ordination) with symbols coloured by system (Warden versus Gore), `r WG.MDS.bySyst$stress`
```{r, include=TRUE}
ggplot(WG.MDS3D.bySyst.pts, aes(x=MDS2, y=MDS3)) + xlim(-0.6, 0.5) + ylim(-0.3, 0.4) + geom_point(aes(colour=SystSeas), size=5) + geom_text(aes(label=Survey), size=3, vjust=1, hjust=1.3)
```

```{r simper analysis of species separating the two systems}
  WG.bySyst.Sim <- simper(WG.bySystem.m, WG.MDS.bySyst.pts$System) #Simper analysis to determine which species best correlated with differences between seasons of whole dataset
...
A simper analysis was undertaken to determine which species were most responsible for the separation of Warden and Gore wetland systems. For the top eight species, box plots of abundance on the Warden versus Gore systems are provided below.  

```{r, include=TRUE}
qplot(x=System, y=Count2, data=agg.spec.data.2, geom="boxplot") + facet_wrap(~ CommonName, scales="free_y", ncol=2) #produce facet wrapped box plots showing differences in top 8 simper species by wetland system
```

##WARDEN SYSTEM ANALYSIS ONLY  

```{r input_data_and_prepare_for_just_Warden_ordinations}
input.data3 <- read.csv("Warden_Gore_by_suite_06_19.csv")
Warden <- input.data3
...
Warden$Count <- sqrt(Warden$Count) #square root data
  ...
  Warden$Survey <- gsub("Oct-2018", "Nov-2018", Warden$Survey) #lump survey dates that straddle Oct/Nov 2018
  yq <- as.yearqtr(as.yearmon(Warden$Date, "%Y/%m/%d"))
...
Warden.sum <- aggregate(data=Warden.sum, Count ~ CommonName + Survey, FUN="sum") #aggregate count data by species and survey
Warden.spr <- aggregate(data=Warden.spr, Count ~ CommonName + Survey, FUN="sum") #aggregate count data by species and survey
```

```{r convert-Warden-only-data-to-seasonal-matrices}
Warden.sum.m <- melt(Warden.sum)
...
Warden.spr.m <- melt(Warden.spr)
Warden.spr.m <- dcast(Warden.spr.m, Survey ~ CommonName, fill="0")
rownames(Warden.spr.m) <- Warden.spr.m[,1]
Warden.spr.m[,1] <- NULL
write.csv(Warden.spr.m, "Warden.spr_m.csv")
Warden.spr.m <- read.csv("Warden.spr_m.csv", row.names=1)
```
...
Ordination graph of Warden system waterbird communities by surveys undertaken in summer, `r Warden.MDS$stress`  
```{r, include=TRUE}
ggplot(Warden.sum.MDS.pts, aes(x=MDS1, y=MDS2)) + xlim(-0.3, 0.5) + ylim(-0.35, 0.3) + coord_fixed(ratio = 1) + geom_point(shape=21, size=7, alpha=0.5, fill="blue", stroke=2) + geom_text(aes(label = Survey), vjust=2, hjust=1) + theme(legend.position = "none")
```
...
ggplot(Warden.spr.MDS.pts, aes(x=MDS1, y=MDS2)) + xlim(-0.3, 0.5) + ylim(-0.35, 0.3) + coord_fixed(ratio = 1) + geom_point(aes(size = depth, colour=depth)) + scale_size(range = c(3, 12)) + geom_text(aes(label = Survey), vjust=2, hjust=1) + theme(legend.position = "none")
```

Ordination graph of Warden system waterbird communities by surveys undertaken in spring  
```{r, include=TRUE}
# note, need to restrict data to just summer, see first code chunk in this section
ggplot(Warden.spr.MDS.pts, aes(x=MDS1, y=MDS2)) + xlim(-0.3, 0.3) + ylim(-0.3, 0.3) + coord_fixed(ratio = 1) + geom_point(shape=21, size=7, alpha=0.5, fill="blue", stroke=2) + geom_text(aes(label = Survey), vjust=2, hjust=1.2) + theme(legend.position = "none")
```
...
ggplot(Warden.spr.MDS.pts, aes(x=MDS1, y=MDS2)) + xlim(-0.3, 0.3) + ylim(-0.25, 0.3) + coord_fixed(ratio = 1) + geom_point(aes(size = depth, colour= depth)) + scale_size(range = c(3, 12)) + geom_text(aes(label = Survey), vjust=2, hjust=1.5) + theme(legend.position = "none")
```
