...
ggplot(WG.3dMDS.bySyst.pts, aes(x=MDS1, y=MDS2)) + xlim(-0.6, 0.5) + ylim(-0.3, 0.4) + geom_point(aes(colour=SystSeas), size=5) + geom_text(aes(label=Survey), size=3, vjust=1, hjust=1.3)
```
...
```{r extract-simperdata-for-top8spp}
...
```{r restrict-countdata-to-simper-top8spp}
...
Warden.spr.m[,1] <- NULL
write.csv(Warden.spr.m, "./analysis_data/Warden.spr_m.csv")
...
```{r 2and3D-ordinations-Wardenonly}
...
row.names(av.depths.spr) <- row.names(Warden.spr.m)
av.depths.spr <- av.depths.spr[, c(3,4)]
write.csv(av.depths.spr, "./analysis_data/av.depths.spr.csv")
av.depths.sum <- av.depths[av.depths$Season == "Summer", ] #restrict to summer
row.names(av.depths.sum) <- row.names(Warden.sum.m)
av.depths.sum <- av.depths.sum[, c(3,4)]
write.csv(av.depths.sum, "./analysis_data/av.depths.sum.csv")
```
...
ggplot(Warden.sum.MDS.pts, aes(x=MDS1, y=MDS2)) + xlim(-0.3, 0.5) + ylim(-0.35, 0.3) + coord_fixed(ratio = 1) + geom_point(aes(size = av.depths.sum$depth, colour=av.depths.sum$depth)) + scale_size(range = c(3, 12)) + geom_text(aes(label = Survey), cex=3, vjust=2, hjust=1.25) + theme(legend.position = "none")
```
...
ggplot(Warden.spr.MDS.pts, aes(x=MDS1, y=MDS2)) + xlim(-0.3, 0.3) + ylim(-0.25, 0.3) + coord_fixed(ratio = 1) + geom_point(aes(size = av.depths.spr$depth, colour=av.depths.spr$depth)) + scale_size(range = c(3, 12)) + geom_text(aes(label = Survey), cex=3, vjust=2, hjust=1.25) + theme(legend.position = "none")
```

Bio-env analysis to determine influence of depth on waterbrd communities on the Warden wetlands
```{r bio-env-Warden}
Warden.be <- bioenv(Warden.spr.m, av.depths.spr, method = "spearman", index = "bray")
Warden.be
```

```{r}
corr(av.depths.spr$depth)
```
