...
Warden.spr.MDS.pts$depth <- as.numeric(av.depths.spr$depth)^3+30 #create new field for depth in the file with mds coordinates
Warden.sum.MDS.pts$depth <- as.numeric(av.depths.sum$depth)^3+30 #create new field for depth in the file with mds coordinates
```
...
ggplot(Warden.sum.MDS.pts, aes(x=MDS1, y=MDS2)) + xlim(-0.3, 0.5) + ylim(-0.35, 0.3) + coord_fixed(ratio = 1) + geom_point(aes(size = depth, colour=depth)) + scale_size(range = c(3, 12)) + geom_text(aes(label = Survey), vjust=2, hjust=1) + theme(legend.position = "none")
```
