...
```{r}
Packages <- c("gridExtra","vegan", "ggrepel","ggplot2", "plyr", "scales", "ggpubr", "stringr","reshape2", "knitr", "zoo", "car","readxl","lubridate")
...
The following is the output from an Rmarkdown R script for analyses of waterbird communities of the Warden and Gore wetland systems surveyed between Oct 2006 and Nov 2020.  
...
input.data <- read.csv("./raw_data/Warden_Gore_by_suite_06_20.csv")
...
rownames(WG.bySystem.m) <- WG.bySystem.m[,1]
  ...
  WG.bySystem.m <- sqrt(WG.bySystem.m)
```
...
#Graphs of Warden system lake depths
```{r}
...
AHD <- as.data.frame (cbind(colnames(Wdepths[, c(2:8)]), c(5.66, 5.19, 3.76, 3.03, 2.783, 2.66, 2.34)))
AHD[, 2] <- as.numeric(AHD[, 2])
Wdepths_AHD <- as.data.frame(Wdepths)
```

##calculate AHD depths
```{r}
for (i in 2:8)
{
  Wdepths_AHD[, i] <- Wdepths[, i]+AHD[i-1, 2]
}
```
...
Warden.spr.MDS <- metaMDS(sqrt(Warden.spr.m), distance = "bray", autotransform = TRUE)
```
